...
Claude Code shows the exact command it will run (`appstrate skills sync --target claude-plugin --target codex --print-path`) and asks you to accept it once. Only published skill versions are synced; publish a version from the Appstrate dashboard to ship a change.
