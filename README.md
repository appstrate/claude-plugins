...
- When the marketplace changes the command it runs (as when `appstrate skills sync` became `appstrate packages sync`), Claude Code stops re-running it in the background until you accept the new one: run `claude plugin update appstrate@appstrate` once. The new command needs an `appstrate` CLI that knows it — `appstrate self-update` (or `npm i -g appstrate@latest`).
